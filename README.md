...
projects start from). Projects depend on `lab-common` rather than duplicating
code.
...
```
lab-common/
├── python/         # pip-installable "labcommon" package
│   ├── src/labcommon/
...
│   └── tests/
├── matlab/
│   ├── +labcommon/  # MATLAB "package" folder (call functions as labcommon.myFunc(...))
...
```bash
pip install -e /path/to/lab-common/python
# or, once pushed to GitHub:
pip install git+https://github.com/<your-username>/lab-common.git#subdirectory=python
```

**MATLAB** — add to path at the top of a project script:
```matlab
addpath(genpath('/path/to/lab-common/matlab'))
```
...
repos can pin to a specific `lab-common` version if needed.
